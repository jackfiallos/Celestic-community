...
class LocaleManager extends CApplicationComponent
{
	...
	{
		if (in_array(strtolower($this->getSelectedLanguage()), Yii::app()->params['languages']))
	...
	public function getFormatByType($format_type, $format_id, $datetime_format=false) 
	{
		...
		$datetime_format =(!empty($datetime_format) ? $datetime_format : Yii::app()->locale->getDateTimeFormat());
	...
	public function toDatabase($dt, $to_format_type=false, $from_format_id=false, $from_format_type=false, $from_format=false, $is_timestamp=false) 
	{
		...
		if (!$is_timestamp) 
		{
			$from_format =(!empty($from_format) ? $from_format : $this->getFormatByType($from_format_type, $from_format_id));
